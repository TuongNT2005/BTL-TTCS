...
export default function EventCard({ item, eventId, onClickFunc }) {
  return (
    <div className="group overflow-hidden rounded-3xl border border-violet-100 bg-white shadow-sm min-w-sm  hover:-translate-y-1 hover:shadow-xl">
      {/* <div className="h-44 bg-gradient-to-br from-violet-500 via-fuchsia-500 to-pink-500" /> */}
      <img src={getImgPath(item.image)} alt="" className="object-fill w-full group-hover:scale-105" />
      <div className="space-y-3 p-5">
        ...
        <div id={eventId} className="flex items-center justify-between gap-3 pt-1">
          ...
          <button onClick={onClickFunc} className="text-sm font-semibold text-violet-700 cursor-pointer">
            Xem chi tiết
          </button>
        </div>
      </div>
