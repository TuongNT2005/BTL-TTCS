...
import { getImgPath, getEventBadgeValue, formatDateInput} from "../../../util"
        ...
        if (isDetailFormOpen) {
            async function fetchData() {
                try {
                    ...
                    res.data.productList.map(product => newDiscountedProducts[product.id] = product.name);
                    ...
                    <form action="" id="update-event-form">
                        ...
                        <section className="h-full flex flex-col md:flex-row justify-center items-center">
                            ...
                            <div className="flex flex-col h-full justify-center items-start p-2 md:p-5 gap-2">
                                <div className="flex flex-row justify-between w-full gap-2">
                                    ...
                                    <input readOnly className="border px-1 py-0.5 rounded-lg border-violet-200" id="id-update-event-form" type="text" defaultValue={formData ? formData.id : ""} />
                                </div>
                                <div className="flex flex-row justify-between w-full gap-2">
                                    <label htmlFor="name-update-event-form" className="font-bold">Tiêu đề: </label>
                                    <input readOnly className="border px-1 py-0.5 rounded-lg border-violet-200" placeholder="Nhập tên sản phẩm..." id="name-update-event-form" type="text" defaultValue={formData ? formData.title : ""} />
                                </div>

                                <div className="flex flex-row justify-between w-full gap-2">
                                    <label htmlFor="discount-update-event-form" className="font-bold">Mức giảm: </label>
                                    <input readOnly className="border px-1 py-0.5 rounded-lg border-violet-200" placeholder="Nhập tên sản phẩm..." id="discount-update-event-form" type="number" defaultValue={formData ? formData.discount : ""} />
                                </div>
                                <div className="flex flex-row justify-between w-full gap-2">
                                    <label htmlFor="startAt-update-event-form" className="font-bold">Ngày bắt đầu: </label>
                                    <input readOnly className="border px-1 py-0.5 rounded-lg border-violet-200" placeholder="Nhập tên sản phẩm..." id="startAt-update-event-form" type="date" defaultValue={formData ? formatDateInput(formData.startAt) : ""} />
                                </div>
                                <div className="flex flex-row justify-between w-full gap-2">
                                    <label htmlFor="endAt-update-event-form" className="font-bold">Ngày kết thúc: </label>
                                    <input readOnly className="border px-1 py-0.5 rounded-lg border-violet-200" placeholder="Nhập tên sản phẩm..." id="endAt-update-event-form" type="date" defaultValue={formData ? formatDateInput(formData.endAt) : ""} />
                                </div>
                                <div className="flex flex-row justify-between w-full gap-2">
                                    <label className="font-bold">Trạng thái: </label>
                                    <Badge value={formData ? getEventBadgeValue(formData.startAt, formData.endAt) : "UNAVALIBLE"} />
                                </div>
                                <div className="flex flex-col justify-center items-start w-full">
                                    <label htmlFor="description-update-event-form" className="font-bold">Mô tả sự kiện: </label>
                                    <textarea readOnly id="description-update-event-form" type="text" placeholder="Nhập mô tả..." defaultValue={formData ? formData.description : ""} className="resize-y  outline-none h-32 w-full border p-1 rounded-lg border-violet-200" />
                                </div>
                            </div>
                        </section>
                        <section className="bg-violet-50 p-2 md:p-5 rounded-2xl">
                            ...
                            <ul className=" p-3 md:p-5 max-h-52 md:max-h-62.5 rounded-2xl">
                                {discountedProducts && Object.keys(discountedProducts).length > 0 ? Object.entries(discountedProducts).map(([productId, productName]) => (
