...
export default function WareHouseSection({ keyword }) {
    ...
    const searchData = useCallback(async function () {
        try {
            setIsLoading(true);
            const res1 = await fetchApiFunc("", `${api.admin.warehouseTab.searchProductVariant}?keyword=${keyword}&page=${curPage}`, "GET", token);
            const res2 = await fetchApiFunc("", api.admin.warehouseTab.getAllProducts, "GET", token);
            console.log(res2);
            setProductVariants(res1.data.content);
            setProductList(res2.data);
            setTotalPage(res1.data.totalPages);
            console.log(res1.data.totalPages);
            return res1;
        } catch (error) {
            alert(error);
            console.error(error)
            ...
            {editFormState.isEditFormOpen ? <ProductVariantEditForm></ProductVariantEditForm> : isImportFormOpen ? <ImportVariantForm></ImportVariantForm> : <></>}
            {
                ...
                !editFormState.isEditFormOpen && !isImportFormOpen && isLoading ? <Loading></Loading> :
                    ...
                    <div className="space-y-6 flex flex-col h-full">
                        <Card className="h-full flex flex-col justify-between" title="Kho" action={<button onClick={onOpenImportForm} className="inline-flex items-center gap-2 rounded-2xl bg-green-500 hover:bg-green-600 px-4 py-2.5 text-sm font-semibold text-white cursor-pointer"><FaPlus size={16} /> Nhập kho</button>}>
