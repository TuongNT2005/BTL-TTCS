...
export default function RefundRequestDetailForm() {
    ...
    function onHandleRequest(status) {
        async function updateRefundStatus() {
            try {
                setIsLoading(true);
                let formData = new FormData();
                formData.set("refundRequestId", refundRequestId);
                formData.set("status", status);
                const res = await fetchApiFunc(formData, api.admin.refundTab.handleRefundRequest, "PUT", token);
