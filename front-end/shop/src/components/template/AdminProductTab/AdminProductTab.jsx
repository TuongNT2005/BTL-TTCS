...
// thứ nhất: khi mà keyword thay đổi thì curpage = 1
// khi curpage thay đổi thì keywword ko đổi
...
export default function AdminProductTab() {
    ...
    const searchData = useCallback(async function (keyword, page) {
        try {
            setIsLoading(true);
            const res = await fetchApiFunc("", `${api.admin.productTab.searchProduct}?keyword=${keyword}&page=${page}`, "GET", token);
            console.log(res);
            setTabData({
    ...
    return <AdminProductContext.Provider value={{ ...editFormState, setEditFormState, setCurPageNumber, refreshKey, setRefreshKey, isCreateFormOpen, setIsCreateFormOpen }}>
